...
if [ "$local_ver" != "$remote_ver" ] || [ ! -f "$INSTALL_DIR/$bin" ]; then
  ...
  wget -O "$bin" "https://github.com/$repo/releases/download/$remote_ver/webssh-linux-amd64"
...
if [ "$local_ver" != "$remote_ver" ] || [ ! -f "$INSTALL_DIR/${bin}-core" ]; then
  ...
  unzip easytier.zip
  chmod +x ./easytier-linux-x86_64/${bin}* && mv ./easytier-linux-x86_64/${bin}* $INSTALL_DIR
  rm -f easytier.zip
  set_local_version "$app" "$remote_ver"
  supervisorctl restart ${bin}-core
  echo "$bin更新完毕已重启"
else
  echo "[$app] 已是最新: $remote_ver"
fi
...
if [ "$local_ver" != "$remote_ver" ] || [ ! -f "$INSTALL_DIR/$bin" ]; then
  echo "[$app] 本地: $local_ver | 最新: $remote_ver → 更新中"
  wget -O uv.tar.gz "https://github.com/$repo/releases/download/$remote_ver/uv-x86_64-unknown-linux-gnu.tar.gz"
